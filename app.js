...
const FUBY = {
  ...
  async requestNotifications() {
    ...
    if (perm === 'granted') {
      ...
      this.showToast('Blocked — go to Chrome Site Settings and Allow notifications for this site.');
    } else {
      this.showToast('Tap Allow when Chrome asks for notification permission.');
    }
  },
  ...
  tellSWtoSchedule() {
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.ready.then(reg => {
        if (reg.active) {
          reg.active.postMessage({ type: 'START_REMINDERS' });
        }
      });
    }
  },
